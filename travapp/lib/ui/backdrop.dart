...
class ItemCards extends StatelessWidget {
  ...
  static List<Widget> _buildFlightCards({int listIndex}) {
    final List<Flight> flightsFly = getFlights(Category.findFlights);
    final List<Flight> flightSleep = getFlights(Category.findFlights);
    final List<Flight> flightEat = getFlights(Category.findFlights);
...
class _BackdropState extends State<Backdrop> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Material(
      child: Scaffold(
        backgroundColor: kTravAppGreen900,
        appBar: AppBar(
...
class TravAppAppBar extends StatefulWidget {
  final Function(int) tabHandler;
  final TabController tabController;

  const TravAppAppBar({Key key, this.tabHandler, this.tabController})
      : super(key: key);

  @override
  _TravAppAppBarState createState() => _TravAppAppBarState();
}

class _TravAppAppBarState extends State<TravAppAppBar> {
  @override
  Widget build(BuildContext context) {
    return SafeArea(
      child: Row(
